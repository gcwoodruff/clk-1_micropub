...
setwd("/Users/gavin/genome/genome/clk-1/to_post_to_github_REVISION/")
...
#the clk-1 expression data for figure 1a; from Woodruff et al. 2024 https://doi.org/10.1093/g3journal/jkae110
...
c <- ggplot(no_exp_a, aes(x=Strain,y=midpoint)) + stat_summary(aes(group=Plate_type),fun = mean, fun.min = mean, fun.max = mean, geom = "crossbar", width = 0.7, colour="black",position = position_dodge(width = 0.9))  + geom_sina(aes(colour=Plate_type),size=1,alpha=0.5) + facet_rep_grid(~Milestone) + theme_half_open()  + background_grid(major = c("x"),minor = c("none")) + theme(strip.background =element_blank(),axis.text.x = element_text(angle = 45, hjust=1)) +scale_colour_manual(values=c("#3B0000","#FF0000")) +scale_y_continuous(limits=c(0,140),breaks=seq(from=0,to=140,by=20)) + ylab("Hours since embryo laid")  + ggtitle("c") + theme(plot.title = element_text(hjust = -0.05))
...
#okay, but for the revision, we add a panel (figure 1E)

#the plot for the revisions-- all the other ubiquinone enzymes
ubiqdat <- read.table("coq_enzyme_tx_data.tsv",header=TRUE,sep="\t")


ubiqdat$gene <- factor(ubiqdat$gene, levels = c("coq-1", "coq-2", "coq-3", "coq-4", "coq-5", "coq-6", "clk-1", "coq-8", "R144.13", "Y62E10A.6"))


cbbPalette <- c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")

ubiqdat$is.sig <- ubiqdat$padj < 0.05

ubiqdat$is.sig <- as.factor(ubiqdat$is.sig)

levels(ubiqdat$is.sig)[levels(ubiqdat$is.sig)=="FALSE"] <- "No"
levels(ubiqdat$is.sig)[levels(ubiqdat$is.sig)=="TRUE"] <- "Yes"

library(ggstats)

ubiqdat$Stage <- as.factor(ubiqdat$Stage)
ubiqdat$Stage <- factor(ubiqdat$Stage, levels = c("L3","L4","Adult"))


ggplot(ubiqdat, aes(x = gene, y = log2FoldChange))  +theme_cowplot()  + geom_stripped_cols(odd = "white", even = "gray90")  + geom_col(aes(fill=Stage,colour=is.sig),position=position_dodge2()) + geom_linerange(aes(ymin = log2FoldChange - lfcSE, ymax = log2FoldChange + lfcSE),position=position_dodge2(0.9)) + scale_fill_manual(values=cbbPalette)  + scale_colour_manual(name= "p < 0.05?",values=c("white","black")) + theme(axis.text.x = element_text(colour="black", size=17,face="italic", angle = 45, hjust = 1)) +xlab("Gene") + ylab(expression( ~Log[2]~paste("fold change"))) 

e <- ggplot(ubiqdat, aes(x = gene, y = log2FoldChange))  +theme_cowplot()  + geom_stripped_cols(odd = "white", even = "gray90")  + geom_col(aes(fill=Stage,colour=is.sig),position=position_dodge2()) + geom_linerange(aes(ymin = log2FoldChange - lfcSE, ymax = log2FoldChange + lfcSE),position=position_dodge2(0.9)) + scale_fill_manual(values=cbbPalette)  + scale_colour_manual(name= "p < 0.05?",values=c("white","black")) + theme(axis.text.x = element_text(colour="black",face="italic", angle = 45, hjust = 1)) +xlab("Gene") + ylab(expression( ~Log[2]~paste("fold change")))  + ggtitle("e") 


(a+b)/(c+d)/e + plot_layout(heights = c(1,1,1.5))
#let's print it see how it looks
ggsave("figure_10-3-24.pdf",width=10, height=12,units="in")
